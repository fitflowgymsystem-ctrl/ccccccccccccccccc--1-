...
export const TrainerFormModal: React.FC<TeamFormModalProps> = ({ editingId, initialData, lang, error, isLoading, onClose, onSave, branches = [] }) => {
    ...
    const renderInput = (label: string, field: string, type: string = 'text', placeholder: string = '', required: boolean = false) => (
        <div className="space-y-1">
            <label className="block text-xs font-black text-gray-400 uppercase tracking-widest ps-1">{label}</label>
            <input
                type={type}
                required={required}
                ...
                className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
            />
        </div>
    );
            ...
            >
                ...
                <div className="px-6 py-4 border-b dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black text-lg text-gray-800 dark:text-white uppercase tracking-widest leading-none">
                                {editingId ? (lang === 'ar' ? 'تعديل البيانات' : 'Edit Member') : (lang === 'ar' ? 'إضافة عضو جديد' : 'Add New Member')}
                            </h3>
                            <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-tighter">Team Database Management</p>
                        </div>
                    </div>
                ...
                <div className="flex px-4 border-b dark:border-slate-700 bg-white dark:bg-slate-800 sticky top-0 z-10">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 py-4 text-sm font-black uppercase tracking-widest transition-all relative flex items-center justify-center gap-2 ${activeTab === tab.id ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                            <tab.icon size={16} />
                        ...
                        <div className="space-y-4 animate-fade-in">
                            ...
                            <div className="space-y-1">
                                ...
                                <textarea
                                    ...
                                    className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500/20 h-20 resize-none"
                                ...
                                <div className="flex gap-2">
                                    {['active', 'inactive'].map(st => (
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-xl text-xs font-black uppercase transition-all ${formData.status === st ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-white dark:bg-slate-900 text-gray-400 border dark:border-slate-700'}`}
                                        >
                        ...
                        <div className="space-y-6 animate-fade-in">
                            {isTrainer ? (
                                <>
                                    ...
                                    <div className="space-y-1">
                                        <label className="block text-xs font-black text-gray-400 uppercase tracking-widest ps-1">{lang === 'ar' ? 'تاريخ التعيين' : 'Hire Date'}</label>
                                        <input type="date" value={formData.hireDate || ''} onChange={e => setFormData({ ...formData, hireDate: e.target.value })} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none h-[38px]" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="block text-xs font-black text-gray-400 uppercase tracking-widest ps-1">{lang === 'ar' ? 'السيرة الذاتية (Short Bio)' : 'Bio / Short Introduction'}</label>
                                        <textarea
                                            value={formData.bio || ''}
                                            onChange={e => setFormData({ ...formData, bio: e.target.value })}
                                            className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500/20 h-24 resize-none"
                                    ...
                                    <div className="p-4 bg-purple-50 dark:bg-purple-900/10 rounded-[1.5rem] border border-purple-100 dark:border-purple-900/20">
                                        <label className="block text-xs font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-3 flex items-center gap-2"><Award size={14} /> Certifications & Qualifications</label>
                                        <textarea
                                            ...
                                            className="w-full px-3 py-2 bg-white dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none focus:ring-2 focus:ring-purple-500/20 h-28 resize-none"
                                ...
                                <>
                                    ...
                                    <div className="space-y-1">
                                        <label className="block text-xs font-black text-gray-400 uppercase tracking-widest ps-1">{lang === 'ar' ? 'تاريخ التعيين' : 'Hire Date'}</label>
                                        <input type="date" value={formData.hireDate || ''} onChange={e => setFormData({ ...formData, hireDate: e.target.value })} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-xl text-base font-bold outline-none" />
                                    </div>
                                    <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-[2rem] border border-amber-100 dark:border-amber-900/20">
                                        <label className="block text-xs font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest mb-3 ps-1">Role Permissions</label>
                    ...
                    {activeTab === 'SECURITY' && (
                        <div className="space-y-6 animate-fade-in">
                            <div className="p-5 bg-slate-950 rounded-[2rem] text-white shadow-xl relative overflow-hidden group">
                                ...
                                <div className="relative z-10 space-y-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-sm font-black text-blue-400 uppercase tracking-widest">Portal Credentials</span>
                                    </div>
                            ...
                            <div className="p-5 bg-gray-50 dark:bg-slate-900/50 rounded-[2rem] border-2 border-dashed border-gray-200 dark:border-slate-700 text-center space-y-3">
                                ...
                                <div>
                                    <h4 className="text-sm font-black text-gray-800 dark:text-white uppercase">{isScanning ? (lang === 'ar' ? 'جاري المسح...' : 'Scanning...') : (lang === 'ar' ? 'تسجيل البصمة' : 'Fingerprint Enrollment')}</h4>
                                    <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest">{isScanning ? (lang === 'ar' ? 'يرجى وضع الإصبع على الجهاز' : 'Please place finger on scanner') : (lang === 'ar' ? 'امسح بصمة الموظف لتسجيل الحضور' : 'Scan staff biometric ID for attendance')}</p>
                                </div>
                                <div className="flex gap-2">
                                    <div className="flex-1 relative">
                                        <input
                                            ...
                                            className={`w-full px-3 py-2 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-xl text-base font-mono outline-none transition-all ${isScanning ? 'ring-2 ring-blue-500/20' : ''}`}
                                        />
                                        {isScanning && (
                                    ...
                                    <button
                                        ...
                                        className={`px-5 py-2 rounded-xl text-sm font-black uppercase shadow-lg transition-all active:scale-95 ${isScanning ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-blue-600/20'}`}
                                    >
                ...
                <div className="p-6 bg-gray-50 dark:bg-slate-950 border-t dark:border-slate-700 flex gap-3">
                    <button onClick={onClose} className="flex-1 py-3 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl text-sm font-black uppercase text-gray-400 tracking-widest hover:bg-gray-100 transition-all active:scale-95 shadow-sm">Cancel</button>
                    <button
                        onClick={handleSubmit}
                        disabled={isLoading}
                        className={`flex-[2] py-4 rounded-2xl font-black text-sm uppercase shadow-2xl transition-all flex items-center justify-center gap-3 tracking-[0.2em] ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-blue-600/30 active:scale-95'}`}
                    >
                        {isLoading ? (
