            ...
            >
                ...
                <div className="px-4 py-3 border-b dark:border-slate-700 flex justify-between items-center bg-green-50 dark:bg-green-900/10">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-black text-lg uppercase tracking-widest text-green-900 dark:text-green-300">{t.wa_campaign}</h3>
                    </div>
                ...
                <div className="p-4 sm:p-6 space-y-4 overflow-y-auto code-scroll">
                    {isSuccess ? (
                        <div className="text-center py-6 animate-fade-in space-y-3">
                            ...
                            <p className="text-sm text-gray-500 font-bold uppercase tracking-widest">Broadcast delivered to {selectedMembers.length} members.</p>
                            <button onClick={onClose} className="px-6 py-2 bg-slate-900 dark:bg-slate-700 text-white rounded-xl font-black text-sm uppercase">Close</button>
                        </div>
                    ) : (
                        <>
                            {/* Filter Section - Status */}
                            <div className="space-y-2">
                                <label className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] ps-1 flex items-center gap-1.5"><Users size={12} /> {t.wa_audience}</label>
                                <div className="grid grid-cols-4 gap-1.5">
                                    {(['ALL', 'ACTIVE', 'EXPIRED'] as const).map(opt => (
                                        <button key={opt} onClick={() => setTarget(opt)} className={`py-1.5 rounded-lg text-sm font-black uppercase border transition-all ${target === opt ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-gray-50 dark:bg-slate-900 text-gray-400 border-transparent'}`}>
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] ps-1 flex items-center gap-1.5"><Clock size={12} /> {t.wa_absence_label}</label>
                                <div className="grid grid-cols-4 gap-1.5">
                                    {(['NONE', '7', '14', '30'] as const).map(opt => (
                                        <button key={opt} onClick={() => setAbsenceFilter(opt)} className={`py-1.5 rounded-lg text-sm font-black uppercase border transition-all ${absenceFilter === opt ? 'bg-amber-500 text-white border-amber-500 shadow-md' : 'bg-gray-50 dark:bg-slate-900 text-gray-400 border-transparent'}`}>
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <label className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] ps-1 flex items-center gap-1.5">
                                        <Users size={12} /> {lang === 'ar' ? 'الأعضاء' : 'Members'} ({filteredRecipients.length})
                                    </label>
                                    <div className="flex gap-1">
                                        <button onClick={selectAll} className="px-2 py-0.5 bg-blue-500 text-white text-xs font-black rounded uppercase">
                                            {lang === 'ar' ? 'الكل' : 'All'}
                                        </button>
                                        <button onClick={selectNone} className="px-2 py-0.5 bg-gray-400 text-white text-[10px] font-black rounded uppercase">
                                            {lang === 'ar' ? 'بلا' : 'None'}
                                        </button>
                                    </div>
                                </div>
                                <div className="max-h-40 overflow-y-auto bg-gray-50 dark:bg-slate-900 rounded-xl p-2 space-y-1 border dark:border-slate-700">
                                    {filteredRecipients.map(user => (
                                        <label key={user.id} className="flex items-center gap-2 p-1.5 hover:bg-white dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-colors">
                                            ...
                                            />
                                            <span className="text-sm font-bold dark:text-white flex-1">{user.name}</span>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] ps-1">{t.wa_message}</label>
                                <textarea
                                    rows={4}
                                    value={message}
                                    onChange={e => setMessage(e.target.value)}
                                    placeholder={lang === 'ar' ? 'اكتب رسالتك هنا... {name}' : 'Message body... {name}'}
                                    className="w-full p-3 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-xl text-base font-bold outline-none focus:ring-2 focus:ring-green-500 shadow-inner dark:text-white resize-none"
                                />
                                <p className="text-xs text-gray-400 font-bold uppercase italic leading-tight">{t.wa_variable_note}</p>
                            </div>
                            ...
                            <div className="bg-green-50 dark:bg-green-900/10 p-2.5 rounded-xl border dark:border-slate-700 flex items-center justify-between">
                                <span className="text-sm font-black dark:text-gray-300 uppercase tracking-widest">{t.wa_recipients_list}</span>
                                <span className="px-2 py-0.5 bg-green-600 text-base font-black rounded-lg shadow-sm">{selectedMembers.length}</span>
                            </div>

                            {selectedMembers.length === 0 && (
                                <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-2 rounded-lg border border-amber-100">
                                    <AlertCircle size={12} />
                                    <span className="text-xs font-black uppercase">{t.wa_no_recipients}</span>
                                </div>
                            )}
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    onClick={onClose}
                                    className="flex-1 py-3 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-300 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-gray-200 transition-all active:scale-95"
                                >
                                    {t.cancel}
                                </button>
                                <button
                                    ...
                                    className="flex-[2] py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-black shadow-lg shadow-green-600/20 active:scale-95 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-2 disabled:opacity-50"
                                >
