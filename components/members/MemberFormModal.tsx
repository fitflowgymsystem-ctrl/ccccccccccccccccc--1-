        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[1000] flex items-center justify-center p-2 sm:p-4 cursor-pointer" onClick={onClose}>
            ...
            >
                <div className="px-4 py-3 border-b dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50 shrink-0">
                    <h3 className="font-black text-[10px] text-gray-800 dark:text-white flex items-center gap-2 uppercase tracking-widest">
                ...
                <form onSubmit={handleSubmit} className="overflow-y-auto p-4 sm:p-5 space-y-4 code-scroll">
                    ...
                    <div className="grid grid-cols-2 gap-3 p-3 bg-gray-50 dark:bg-slate-900/30 rounded-xl border dark:border-slate-700">
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.dob || 'Date of Birth'}</label>
                            <input type="date" value={formData.dob} onChange={e => setFormData({ ...formData, dob: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] uppercase font-bold outline-none dark:text-white" />
                        </div>
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.gender}</label>
                            <select value={formData.gender} onChange={e => setFormData({ ...formData, gender: e.target.value as Gender })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] uppercase font-bold outline-none dark:text-white">
                                <option value={Gender.MALE}>{t.male}</option>
                                <option value={Gender.FEMALE}>{t.female}</option>
                            </select>
                        </div>
                        <div className="space-y-1 col-span-2">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.branch || 'Branch'}</label>
                            <select value={formData.branch} onChange={e => setFormData({ ...formData, branch: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] uppercase font-bold outline-none dark:text-white">
                                {branches.length > 0 ? (
                        ...
                        <div className="col-span-2 grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                                <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.emergency_name || 'Emergency Contact Name'}</label>
                                <input type="text" placeholder="Name" value={formData.emergencyContactName} onChange={e => setFormData({ ...formData, emergencyContactName: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-bold outline-none dark:text-white" />
                            </div>
                            <div className="space-y-1">
                                <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.emergency_phone || 'Emergency Phone'}</label>
                                <input type="tel" placeholder="Phone" value={formData.emergencyContactPhone} onChange={e => setFormData({ ...formData, emergencyContactPhone: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-bold outline-none dark:text-white" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.membership}</label>
                            ...
                            }} className="w-full px-2 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white font-bold">
                                {Object.values(MembershipType).map(v => <option key={v} value={v}>{v}</option>)}
                            </select>
                        </div>
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.select_offer}</label>
                            <select value={formData.activeOfferId} onChange={e => setFormData({ ...formData, activeOfferId: e.target.value })} className="w-full px-2 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white font-bold">
                    ...
                    <div className="space-y-3 p-3 bg-indigo-50/30 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-slate-700">
                        <label className="text-[9px] font-black text-indigo-700 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1.5 mb-1">
                            <Gift size={12} /> {t.training_perks || 'Training Perks'}
                        </label>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            <div className="space-y-1">
                                <label className="flex items-center gap-1.5 text-[8px] font-black text-purple-500 uppercase tracking-widest">
                                    <Activity size={10} /> {t.perks_inbody}
                                </label>
                                <input type="number" min="0" placeholder="0" value={formData.inbodySessions ?? ''} onChange={e => setFormData({ ...formData, inbodySessions: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                            </div>
                            <div className="space-y-1">
                                <label className="flex items-center gap-1.5 text-[8px] font-black text-orange-500 uppercase tracking-widest">
                                    <Gift size={10} /> {t.perks_guest}
                                </label>
                                <input type="number" min="0" placeholder="0" value={formData.guestPasses ?? ''} onChange={e => setFormData({ ...formData, guestPasses: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                            </div>
                            <div className="space-y-1">
                                <label className="flex items-center gap-1.5 text-[8px] font-black text-blue-500 uppercase tracking-widest">
                                    <Dumbbell size={10} /> {t.pt_sessions || 'PT Sessions'}
                                </label>
                                <input type="number" min="0" placeholder="0" value={formData.ptSessions ?? ''} onChange={e => setFormData({ ...formData, ptSessions: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                            </div>
                            <div className="col-span-2 space-y-1">
                                <label className="flex items-center gap-1.5 text-[8px] font-black text-green-500 uppercase tracking-widest">
                                ...
                                <div className="grid grid-cols-5 gap-1.5">
                                    <input
                                        ...
                                        className="col-span-2 px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white"
                                    />
                                    <select
                                        value={formData.freeGroupClassId}
                                        onChange={e => setFormData({ ...formData, freeGroupClassId: e.target.value })}
                                        className="col-span-3 px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white font-bold"
                                    >
                    ...
                    <div className="space-y-3 p-3 bg-gray-50/50 dark:bg-slate-900/50 rounded-xl border dark:border-slate-700">
                        <label className="text-[9px] font-black text-gray-700 dark:text-gray-400 uppercase tracking-widest flex items-center gap-1.5 mb-1">
                    ...
                    <div className="space-y-2 p-3 bg-green-50/50 dark:bg-green-900/10 rounded-xl border border-green-100 dark:border-green-800/30">
                        <label className="text-[9px] font-black text-green-700 dark:text-green-500 uppercase tracking-widest flex items-center gap-1.5">
                            <Activity size={12} /> {t.health_info || 'Health & Fitness'}
                        </label>
                        <div className="grid grid-cols-3 gap-2">
                            <input type="number" placeholder="Weight (kg)" value={formData.weight ?? ''} onChange={e => setFormData({ ...formData, weight: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                            <input type="number" placeholder="Height (cm)" value={formData.height ?? ''} onChange={e => setFormData({ ...formData, height: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                            <input type="number" placeholder="Fat %" value={formData.fatPercentage ?? ''} onChange={e => setFormData({ ...formData, fatPercentage: e.target.value === '' ? null : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white" />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            <select value={formData.fitnessGoal} onChange={e => setFormData({ ...formData, fitnessGoal: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white">
                            ...
                            <select value={formData.bloodType} onChange={e => setFormData({ ...formData, bloodType: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white">
                        ...
                        <textarea placeholder="Medical Conditions / Injuries..." value={formData.medicalConditions} onChange={e => setFormData({ ...formData, medicalConditions: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] outline-none dark:text-white resize-none h-14" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            ...
                            }} className="w-full px-2 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-mono dark:text-white" />
                        </div>
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.expiry}</label>
                            <input type="date" value={formData.expiryDate} onChange={e => setFormData({ ...formData, expiryDate: e.target.value })} className="w-full px-2 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-mono dark:text-white" />
                        </div>
                    </div>
                    ...
                    {!editingMember && (
                        <div className="grid grid-cols-2 gap-3 p-3 bg-amber-50/50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-800/30">
                            <div className="space-y-1">
                                ...
                                <select value={formData.paymentMethod} onChange={e => setFormData({ ...formData, paymentMethod: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-bold outline-none dark:text-white">
                            ...
                            <div className="space-y-1">
                                ...
                                <input type="number" placeholder="Paid Amount" value={formData.totalPaid ?? ''} onChange={e => setFormData({ ...formData, totalPaid: e.target.value === '' ? undefined : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-bold outline-none dark:text-white" />
                            </div>
                        </div>
                    )}
                    <div className={`p-4 rounded-xl border-2 transition-all ${formData.installmentPlan.enabled ? 'bg-orange-50 dark:bg-orange-950/10 border-orange-200' : 'bg-gray-50 dark:bg-slate-900/50 border-gray-100 dark:border-slate-700'}`}>
                        <div className="flex items-center justify-between mb-3">
                            <label className="text-[10px] font-black text-orange-700 dark:text-orange-500 uppercase tracking-widest flex items-center gap-1.5">
                                <DollarSign size={14} /> {lang === 'ar' ? 'نظام التقسيط' : 'Installment Plan'}
                            </label>
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-3 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-xs font-bold outline-none"
                                            placeholder="0.00"
                                        />
                                    </div>
                                    <div className="space-y-1">
                    ...
                    <div className={`p-3 rounded-xl border transition-all ${formData.isPrivate ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-200' : 'bg-gray-50 dark:bg-slate-900/50 border-gray-100 dark:border-slate-700'}`}>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-[9px] font-black text-amber-700 dark:text-amber-500 uppercase tracking-widest flex items-center gap-1.5">
                                <Crown size={12} /> {t.private_training}
                            </label>
                        ...
                        {formData.isPrivate && (
                            <div className="grid grid-cols-2 gap-2 animate-fade-in">
                                <select value={formData.assignedTrainerId} onChange={e => setFormData({ ...formData, assignedTrainerId: e.target.value })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border border-amber-200 rounded-lg text-[10px] outline-none dark:text-white">
                                    <option value="">Coach</option>
                                    {trainers.map(tr => <option key={tr.id} value={tr.id}>{tr.name}</option>)}
                                </select>
                                <input type="number" placeholder="Price" value={formData.privateSessionPrice ?? ''} onChange={e => setFormData({ ...formData, privateSessionPrice: e.target.value === '' ? undefined : Number(e.target.value) })} className="w-full px-2 py-1.5 bg-white dark:bg-slate-950 border border-amber-200 rounded-lg text-[10px] outline-none dark:text-white font-bold" />
                            </div>
                        )}
                    </div>

                    <div className="p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/30">
                        <label className="block text-[8px] font-black text-blue-700 dark:text-blue-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                        ...
                        <div className="flex gap-2">
                            <div className="flex-1 relative">
                                <input
                                    ...
                                    className={`w-full px-3 py-1.5 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-lg text-[10px] font-mono dark:text-white outline-none transition-all ${isScanning ? 'ring-2 ring-blue-500/20' : ''}`}
                                />
                                {isScanning && (
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-1 transition-all active:scale-95 shadow-md tracking-widest ${isScanning ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-blue-600/20 hover:bg-blue-700'}`}
                            >
                    ...
                    <div className="pt-2 sticky bottom-0 bg-white dark:bg-slate-800 py-3 border-t dark:border-slate-700 z-10 flex gap-2">
                        <button type="button" onClick={onClose} className="flex-1 py-2.5 bg-gray-100 dark:bg-slate-700 text-gray-500 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-200 active:scale-95 transition-all">Cancel</button>
                        <button type="submit" className="flex-[2] py-2.5 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95 transition-all tracking-widest">
                            <Save size={16} /> {t.save}
                        </button>
                    </div>
                </form>
