...
export const ScannerUI: React.FC<ScannerUIProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 flex flex-col items-center justify-center space-y-8 relative overflow-hidden">
            ...
            <div className={`relative w-48 h-48 rounded-full border-4 flex items-center justify-center transition-all duration-500 ${scanning ? 'border-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.5)]' :
            ...
            <div className="w-full max-w-sm space-y-4">
                <CustomSelect
                    label={t.select_finger}
                    value={selectedUser}
                    onChange={val => { setSelectedUser(val); if (result) reset(); }}
                    options={[
                        { label: `-- ${t.select_finger} --`, value: '', icon: <Fingerprint size={14} className="text-gray-400" /> },
                        ...users.map(u => ({ label: `Finger ID: ${u.fingerprintId} (${u.name})`, value: u.id, icon: <Fingerprint size={14} className="text-blue-500" /> })),
                        { label: t.unknown_finger, value: '999', icon: <Fingerprint size={14} className="text-red-500" /> }
                    ]}
                />
