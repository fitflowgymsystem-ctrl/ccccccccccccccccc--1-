            ...
            >
                ...
                <div className="px-4 py-3 border-b dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50 text-start">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs font-bold text-gray-400 mt-1 uppercase tracking-tighter">Assigning: {service.name}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2 text-start">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 text-gray-900 dark:text-white rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-mono"
                            />
                        </div>
                    </div>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-start">
                                            <p className={`text-sm font-black truncate max-w-[180px] ${selectedUserId === user.id ? 'text-white' : 'dark:text-white'}`}>{user.name}</p>
                                            <p className={`text-[11px] font-bold uppercase ${selectedUserId === user.id ? 'text-white/60' : 'text-gray-400'}`}>{user.phone}</p>
                                        </div>
                                    </div>
                    ...
                    {selectedUserId && (
                        <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-[1.5rem] border border-gray-100 dark:border-slate-700 space-y-3 animate-fade-in text-start">
                            <h4 className="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest flex items-center gap-2">
                                <Hash size={12} /> Subscription Summary
                            </h4>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase">Sessions</p>
                                    <p className="text-base font-black text-gray-900 dark:text-white">
                                        {service.pricingType === 'SUBSCRIPTION' ? (lang === 'ar' ? 'غير محدود' : 'Unlimited') : (service.packageSessions || 1)}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase">Validity</p>
                                    <p className="text-base font-black text-gray-900 dark:text-white">{service.validityDays || 0} Days</p>
                                </div>
                                <div className="col-span-2 pt-2 border-t border-blue-100 dark:border-blue-500/30">
                                    <p className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase">Total Price</p>
                                    <p className="text-lg font-black text-blue-600 dark:text-blue-400">{service.price} EGP</p>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-6 bg-gray-50 dark:bg-slate-950 border-t dark:border-slate-700 flex gap-3">
                    ...
                    <button
                        ...
                        className={`flex-[2] py-3 rounded-2xl font-black text-sm uppercase shadow-2xl transition-all flex items-center justify-center gap-3 tracking-[0.2em] ${!selectedUserId ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-blue-600/30 active:scale-95'}`}
                    >
