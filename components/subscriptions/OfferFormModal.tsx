        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[110] p-4 animate-fade-in cursor-pointer" onClick={onClose}>
            ...
            >
                <div className="px-4 py-3 border-b dark:border-slate-700 flex justify-between items-center bg-purple-50/50 dark:bg-slate-900/50 shrink-0">
                    <h3 className="font-black text-[10px] uppercase tracking-widest text-purple-900 dark:text-purple-300 flex items-center gap-2">
                ...
                <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-3">
                    <div className="space-y-1">
                        ...
                        <input type="text" required placeholder="Summer Sale..." value={offerForm.title} onChange={e => setOfferForm({ ...offerForm, title: e.target.value })} className="w-full px-3 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-xs font-bold text-gray-900 dark:text-white outline-none focus:ring-1 focus:ring-purple-500" />
                    </div>

                    <div className="space-y-1">
                        <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.offer_code}</label>
                        <input type="text" required placeholder="OFF50" value={offerForm.code} onChange={e => setOfferForm({ ...offerForm, code: e.target.value.toUpperCase() })} className="w-full px-3 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-xs font-black text-purple-600 dark:text-purple-400 outline-none uppercase tracking-widest" />
                    </div>

                    <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.offer_value}</label>
                            <input type="number" required min="1" placeholder="--" value={offerForm.discountValue ?? ''} onChange={e => setOfferForm({ ...offerForm, discountValue: e.target.value === '' ? undefined : Number(e.target.value) })} className="w-full px-3 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-xs font-black dark:text-white outline-none" />
                        </div>
                        <div className="space-y-1">
                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.discount_type}</label>
                            <select value={offerForm.discountType} onChange={e => setOfferForm({ ...offerForm, discountType: e.target.value as any })} className="w-full px-2 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-black outline-none dark:text-white" >
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <input type="date" required value={offerForm.validUntil} onChange={e => setOfferForm({ ...offerForm, validUntil: e.target.value })} className="w-full pl-8 pr-3 py-1.5 bg-gray-50 dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-xs font-bold dark:text-white outline-none" />
                        </div>
                    </div>
                    ...
                    <div className="pt-2 flex gap-2">
                        ...
                        <button type="submit" className="flex-[2] py-2 bg-purple-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-purple-600/20 active:scale-95 transition-all flex items-center justify-center gap-2">
                            <Save size={14} /> {t.save}
                        </button>
                    </div>
                </form>
