            ...
            >
                <div className="px-5 py-3 border-b dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 className="font-black text-[10px] dark:text-white uppercase tracking-widest">{editingService ? t.edit : t.add_service}</h3>
                            <p className="text-[8px] text-gray-400 font-bold uppercase tracking-tighter leading-none mt-0.5">{formData.category}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4 code-scroll">
                    ...
                    <div className="grid grid-cols-12 gap-3">
                        <div className="col-span-12 md:col-span-6 space-y-1">
                            ...
                            <input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-900 border dark:border-slate-700 rounded-lg text-[11px] font-bold outline-none focus:border-blue-500/50 transition-all dark:text-white" />
                        </div>
                    ...
                    <div className="grid grid-cols-12 gap-3">
                        ...
                        <div className="col-span-12 md:col-span-4 space-y-1">
                            ...
                            <div className="flex bg-gray-50 dark:bg-slate-950 p-0.5 rounded-lg border dark:border-slate-700">
                                <button type="button" onClick={() => setFormData({ ...formData, status: 'AVAILABLE' })} className={`flex-1 py-1.5 rounded-md text-[8px] font-black uppercase transition-all ${formData.status === 'AVAILABLE' ? 'bg-white dark:bg-slate-800 text-green-600 shadow-sm' : 'text-gray-400'}`}>{t.available}</button>
                                <button type="button" onClick={() => setFormData({ ...formData, status: 'UNAVAILABLE' })} className={`flex-1 py-1.5 rounded-md text-[8px] font-black uppercase transition-all ${formData.status === 'UNAVAILABLE' ? 'bg-white dark:bg-slate-800 text-red-600 shadow-sm' : 'text-gray-400'}`}>{t.unavailable}</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-3.5 bg-gray-50/50 dark:bg-slate-900/50 rounded-2xl border dark:border-slate-700 space-y-3">
                        <div className="flex items-center justify-between">
                            <label className="text-[8px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                <DollarSign size={12} /> {t.pricing_type}
                            </label>
                            <div className="flex gap-1">
                                {(['PER_SESSION', 'PACKAGE', 'SUBSCRIPTION'] as ServicePricingType[]).map(type => (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-md text-[7px] font-black uppercase border transition-all ${formData.pricingType === type ? 'bg-blue-600 text-white border-blue-700' : 'bg-white dark:bg-slate-800 text-gray-400 border-gray-100 dark:border-slate-700'}`}
                                    >
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="space-y-1">
                                <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.price}</label>
                                <div className="relative">
                                    <input type="number"
                                        value={formData.price}
                                        placeholder="0"
                                        onChange={e => setFormData({ ...formData, price: e.target.value === '' ? '' as any : Number(e.target.value) })}
                                        className="w-full pl-3 pr-8 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[11px] font-bold outline-none focus:border-blue-500/30 dark:text-white"
                                        dir="ltr"
                                    />
                                    <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[7px] font-black text-gray-300 pointer-events-none">EGP</span>
                                </div>
                            </div>
                            {formData.pricingType !== 'PER_SESSION' && (
                                <div className="space-y-1">
                                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.validity_days}</label>
                                    <input type="number"
                                        value={formData.validityDays}
                                        placeholder="0"
                                        onChange={e => setFormData({ ...formData, validityDays: e.target.value === '' ? '' as any : Number(e.target.value) })}
                                        className="w-full px-3 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[11px] font-bold outline-none focus:border-blue-500/30 dark:text-white"
                                    />
                                </div>
                            )}
                            {formData.pricingType === 'PACKAGE' && (
                                <div className="space-y-1">
                                    <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ps-1">{t.package_sessions}</label>
                    ...
                    {formData.category === 'Group Class' && (
                        <div className="p-3.5 bg-indigo-50/30 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100/50 dark:border-slate-700 space-y-3">
                            <label className="text-[8px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                <Users size={12} /> {t.group_classes} INFO
                            </label>
                            <div className="grid grid-cols-12 gap-3">
                                <div className="col-span-12 md:col-span-6 space-y-1">
                                ...
                                <div className="col-span-6 md:col-span-3 space-y-1">
                                    ...
                                    <input value={formData.room} onChange={e => setFormData({ ...formData, room: e.target.value })} className="w-full px-3 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[11px] font-bold outline-none dark:text-white" />
                                </div>
                                    ...
                                    <div className="flex items-center gap-3 mt-2">
                                        <div className="flex-1 space-y-1">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-3 py-1.5 bg-white dark:bg-slate-950 border dark:border-slate-700 rounded-lg text-[10px] font-bold dark:text-white outline-none focus:border-indigo-500/50"
                                            />
                                        </div>
                                        <div className="flex-[2] pt-3">
                                            <div className="px-3 py-2 bg-indigo-50/50 dark:bg-slate-950 rounded-lg border border-dashed border-indigo-200 dark:border-slate-800">
                                                <p className="text-[8px] font-black text-indigo-400 uppercase tracking-tighter">Preview:</p>
                                                <p className="text-[10px] font-bold text-indigo-600 dark:text-indigo-300 truncate">
                ...
                <div className="p-3 bg-gray-50/50 dark:bg-slate-900/50 border-t dark:border-slate-700 flex gap-2">
                    <button onClick={onClose} type="button" className="flex-1 py-2.5 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl text-[9px] font-black uppercase text-gray-500 tracking-widest active:scale-95 transition-all">Cancel</button>
                    <button onClick={handleSubmit} type="button" className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-blue-600/20 active:scale-95 transition-all flex items-center justify-center gap-2">
