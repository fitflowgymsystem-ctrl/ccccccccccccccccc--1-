            ...
            >
                <div className="p-8 border-b dark:border-slate-700 flex justify-between items-center bg-amber-50 dark:bg-amber-900/20">
                    <div>
                        ...
                        <p className="text-sm text-amber-600 font-bold uppercase tracking-widest mt-1">{t.dev_broadcast_sub}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                {(['info', 'warning', 'urgent'] as const).map(type => (
                                    <button key={type} type="button" onClick={() => setNotifData({ ...notifData, type })} className={`flex-1 py-2 rounded-xl text-sm font-black uppercase tracking-widest border transition-all ${notifData.type === type ? 'bg-amber-500 text-white border-amber-500 shadow-md' : 'bg-gray-50 dark:bg-slate-900 text-gray-400 border-transparent'}`}>
                                        {type}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <input required type="text" value={notifData.title} onChange={e => setNotifData({ ...notifData, title: e.target.value })} className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-950 border-none rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-amber-500 shadow-inner dark:text-white" />
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 block ps-1">{t.wa_message}</label>
                            <textarea required rows={4} value={notifData.message} onChange={e => setNotifData({ ...notifData, message: e.target.value })} className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-950 border-none rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-amber-500 shadow-inner dark:text-white resize-none" />
                        </div>
                    </div>
                    <button type="submit" className="w-full py-5 bg-amber-600 text-white rounded-2xl font-black shadow-xl shadow-amber-900/40 active:scale-95 transition-all uppercase tracking-widest text-base flex items-center justify-center gap-2">
